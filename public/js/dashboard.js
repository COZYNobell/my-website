            ...
            favorites.forEach(fav => {
                ...
                deleteButton.addEventListener('click', async function() {
                    if (confirm(`'${this.dataset.name}' 즐겨찾기를 삭제하시겠습니까?`)) {
                        try {
                            // 수정된 부분: API_GATEWAY_BASE_URL 사용
                            await fetchData(`<span class="math-inline">\{API\_GATEWAY\_BASE\_URL\}/favorites/</span>{this.dataset.id}`, { method: 'DELETE' });
...
async function fetchAndDisplayWeather(lat, lon, locationName = null) {
    ...
    try {
        const [currentWeatherData, forecastData] = await Promise.all([
            // 수정된 부분: API_GATEWAY_BASE_URL 사용
            fetchData(`<span class="math-inline">\{API\_GATEWAY\_BASE\_URL\}/weather\-by\-coords?lat\=</span>{lat}&lon=${lon}`),
            fetchData(`<span class="math-inline">\{API\_GATEWAY\_BASE\_URL\}/weather\-forecast?lat\=</span>{lat}&lon=${lon}`)
        ]);
        if (currentWeatherData) {
            weatherTodayP.innerHTML = `
                <strong>${locationName || currentWeatherData.cityName || '알 수 없는 지역'}</strong><br>
                상태: <span class="math-inline">\{currentWeatherData\.description \|\| '정보 없음'\} <img src\="http\://openweathermap\.org/img/wn/</span>{currentWeatherData.icon || '01d'}.png" alt="날씨 아이콘" style="vertical-align: middle;"><br>
                ...
                forecastData.forecast.forEach(day => {
                    forecastHtml += `
                        <div style="margin-top: 5px; padding-top: 5px; border-top: 1px solid #efefef;">
                            <strong>${day.date}</strong>: <span class="math-inline">\{day\.description \|\| '정보 없음'\} <img src\="http\://openweathermap\.org/img/wn/</span>{day.icon || '01d'}.png" alt="날씨 아이콘" style="vertical-align: middle;"><br>
