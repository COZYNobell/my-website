...
// ✨ SQLite 데이터베이스 설정 및 users 테이블 생성 ✨
...
// ... (기존 /weather, /api/weather-by-coords, /api/weather-forecast 라우트는 동일하게 유지) ...
...
app.get('/api/weather-forecast', async (req, res) => {
  ...
  try {
    ...
    forecastData.list.forEach(item => { /* ... (이전 예보 데이터 가공 로직과 동일) ... */
      const date = item.dt_txt.split(' ')[0]; if (!dailyForecasts[date]) { dailyForecasts[date] = { temps: [], weather_descriptions: [], icons: [], dt_txts: [] }; }
      dailyForecasts[date].temps.push(item.main.temp); dailyForecasts[date].weather_descriptions.push(item.weather[0].description); dailyForecasts[date].icons.push(item.weather[0].icon); dailyForecasts[date].dt_txts.push(item.dt_txt);
    });
    const processedForecast = []; const today = new Date(); let addedDays = 0; const datesProcessed = new Set();
    for (const item of forecastData.list) {
      const itemDateStr = item.dt_txt.split(' ')[0]; const itemDate = new Date(itemDateStr); itemDate.setHours(0,0,0,0);
      ...
      if (itemDate > todayDateOnly && !datesProcessed.has(itemDateStr) && addedDays < 2) {
        const dayData = dailyForecasts[itemDateStr]; let representativeIndex = Math.floor(dayData.dt_txts.length / 2);
        const specificTimeIndex = dayData.dt_txts.findIndex(dt_txt => dt_txt.includes("15:00:00")); if (specificTimeIndex !== -1) representativeIndex = specificTimeIndex;
...
// ✨✨✨ [수정됨] 사용자 회원가입 처리 라우트 (/signup) - SQLite 사용 ✨✨✨
app.post('/signup', async (req, res) => {
  try {
    ...
    db.get("SELECT * FROM users WHERE email = ?", [email], async (err, row) => {
      if (err) {
        console.error("회원가입 중 DB 조회 오류:", err.message);
        return res.status(500).send('회원가입 처리 중 오류가 발생했습니다. <a href="/signup.html">다시 시도</a>');
      }
      if (row) { // 이미 사용자가 존재하면
        return res.status(409).send('이미 가입된 이메일입니다. <a href="/login.html">로그인</a> 하시거나 다른 이메일로 가입해주세요. <a href="/signup.html">다시 시도</a>');
      }

      // 사용자가 존재하지 않으면 비밀번호 암호화 및 DB에 저장
      const saltRounds = 10;
      ...
      db.run("INSERT INTO users (email, password) VALUES (?, ?)", [email, hashedPassword], function(err) {
        if (err) {
          console.error("회원가입 중 DB 삽입 오류:", err.message);
          return res.status(500).send('회원가입 처리 중 오류가 발생했습니다. <a href="/signup.html">다시 시도</a>');
        }
        console.log(`새로운 사용자 가입됨 (DB ID: ${this.lastID}, email: ${email})`);
        // DB에서 새로 가입된 사용자 정보를 다시 가져와서 로그에 찍을 수도 있습니다 (선택 사항)
        // db.all("SELECT id, email FROM users", [], (err, rows) => {
        //   if (err) throw err;
        //   console.log('현재 가입된 사용자 목록 (DB):', rows);
        // });
        res.redirect('/login.html?signup=success');
      });
    });
  } catch (error) { // 이 catch는 bcrypt.hash에서 예외 발생 시 주로 동작
    console.error("회원가입 처리 중 예기치 않은 오류:", error);
    res.status(500).send('회원가입 처리 중 오류가 발생했습니다. <a href="/signup.html">다시 시도</a>');
  }
});

// ✨✨✨ [수정됨] 사용자 로그인 처리 라우트 (/login) - SQLite 사용 ✨✨✨
app.post('/login', (req, res) => { // bcrypt.compare는 async이므로, 전체 함수를 async로 만들거나 콜백 내에서 처리
  try {
    const { email, password } = req.body;
    if (!email || !password) {
    ...
    // ✨ DB에서 사용자 찾기 ✨
    db.get("SELECT * FROM users WHERE email = ?", [email], async (err, user) => { // ✨ 콜백을 async로 변경
      if (err) {
        console.error("로그인 중 DB 조회 오류:", err.message);
        return res.status(500).send('로그인 처리 중 오류가 발생했습니다. <a href="/login.html">다시 시도</a>');
      }
      if (!user) {
      ...
      req.session.user = {
        id: user.id, // ✨ DB에서 가져온 id 사용
        email: user.email
      };
      console.log('사용자 로그인 성공:', req.session.user);
      res.redirect('/dashboard.html');
    });
  } catch (error) { // 이 catch는 req.body 접근 등 동기 코드에서 오류 발생 시 주로 동작
    console.error("로그인 처리 중 예기치 않은 오류:", error);
...
// 6.8. API: 현재 로그인된 사용자 정보 반환 (/api/current-user)
